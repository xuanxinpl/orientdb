...
public abstract class OAbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {

  private static final long serialVersionUID = 7373984972572414691L;

  /**
   * Creates a new {@code OAbstractQueuedSynchronizer} instance
   * with initial synchronization state of zero.
   */
  protected OAbstractQueuedSynchronizer() {
  }
   ...
   * CANCELLED.
   */
  private transient final AtomicReference<Node> head = new AtomicReference<Node>();

  /**
   * Tail of the wait queue, lazily initialized.  Modified only via
   * method enq to add new wait node.
   */
  private transient final AtomicReference<Node> tail = new AtomicReference<Node>();
  ...
  protected final boolean compareAndSetState(int expectedState, int newState) {
  ...
  private void cancelAcquire(Node node) {
    ...
    if (node == tail.get() && tail.compareAndSet(node, pred)) {
      ...
      int ws;
      if (pred != head.get() &&
          ((ws = pred.waitStatus.get()) == Node.SIGNAL || (ws <= 0 && pred.waitStatus.compareAndSet(ws, Node.SIGNAL))) &&
          pred.thread != null) {
      ...
      node.next.lazySet(node); // help GC
    }
  }
  ...
  private Thread fullGetFirstQueuedThread() {
    ...
    */
    ...
    if (((h = head.get()) != null && (s = h.next.get()) != null &&
        s.prev == head.get() && (st = s.thread) != null) || ((h = head.get()) != null && (s = h.next.get()) != null &&
   ...
   * avoid <a href="OAbstractQueuedSynchronizer#barging">barging</a>.
  ...
  public final boolean hasQueuedPredecessors() {
    ...
    Node h = head.get();
    Node s;
    return h != t && ((s = h.next.get()) == null || s.thread != Thread.currentThread());
  }
   ...
   * OAbstractQueuedSynchronizer} serving as the basis of a {@link
   * Lock} implementation.
   * <p>
   ...
   * {@code OAbstractQueuedSynchronizer}.
   * <p>
  ...
  public class ConditionObject implements Condition, java.io.Serializable {
     ...
     * @return {@code true} if owned
     */
    final boolean isOwnedBy(OAbstractQueuedSynchronizer sync) {
      return sync == OAbstractQueuedSynchronizer.this;
    }

    /**
     * Queries whether any threads are waiting on this condition.
     * Implements {@link OAbstractQueuedSynchronizer#hasWaiters(ConditionObject)}.
     *
     * @return {@code true} if there are any waiting threads
     * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
     *                                      returns {@code false}
     ...
     * this condition.
     * Implements {@link OAbstractQueuedSynchronizer#getWaitQueueLength(ConditionObject)}.
     *
     * @return the estimated number of waiting threads
     * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
     *                                      returns {@code false}
     ...
     * Implements {@link OAbstractQueuedSynchronizer#getWaitingThreads(ConditionObject)}.
     *
     * @return the collection of threads
     * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
     *                                      returns {@code false}
